...
def main():
    ...
    while K < pruning_num:
        print(f"K = {K}")
        model,flag = prune(model,original_model,layer_names, train_loader, val_loader, L_cls_f,dictionary,dictionary_input,k)
        if flag != -1:
            K += k
        loss, acc = validate(val_loader, model, L_cls_f, '* ')
        print("[Pruning done: {:2.2f}%]. Loss: {:.3f}. Acc: {:2.2f}%. || Param: {:2.2f}%  Flop: {:2.2f}%".format(K/pruning_num*100, loss,
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(state, './checkpoint/pruned_cifar10_lrf.pth')
...
def prune(model,original_model,layer_names, train_loader, val_loader, L_cls_f,dictionary,dictionary_input,k):
    best_model = model
    best_acc = 0
    model_copy = copy.deepcopy(model)
    best_idx = -1
    flag = -1
    print("Layer Indexes for which pruning is finished:",end =" ")
    for i in range(1,len(layer_names)):
      layer_name = layer_names[i]
      model_copy1 = copy.deepcopy(model_copy)
      model_copy2 = copy.deepcopy(model_copy)

      model_out,pruned_out = pruning_output_channel(model_copy1, original_model, layer_name, train_loader, val_loader, L_cls_f, dictionary,k)
      model_in,pruned_in = pruning_input_channel(model_copy2, original_model, layer_name, train_loader, val_loader, L_cls_f, dictionary_input,k)
    ...
    return(best_model,flag)

def pruning_output_channel(model, original_model, layer_name, train_loader, val_loader, L_cls_f, dictionary,k):
    ...
    for xx in range(num):
        ...
        if len(S)==1:
            ...
            lambda_id = lambda_id.view(lambda_id.shape[0],-1)
        else:
            lambda_id = torch.from_numpy(lambda_list[S_das,:])
        ...
        # print("compen_weight shape:", compen_weight.shape)
...
def pruning_input_channel(model, original_model, layer_name, train_loader, val_loader, L_cls_f,dictionary_input,k):
    ...
    for xx in range(num):
        ...
        if len(S)==1:
            lambda_id = torch.from_numpy(lambda_list[S_das])
            lambda_id = lambda_id.view(-1,lambda_id.shape[0])
        else:
            lambda_id = torch.from_numpy(lambda_list[:,S_das])
